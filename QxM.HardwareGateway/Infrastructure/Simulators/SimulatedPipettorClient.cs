...
public sealed class SimulatedPipettorClient : IHardwareClient<ApiCommandRequest>
{
    ...
    private readonly string _firmwareVersion = "sim-pipettor-1.0.1.0";
    
    public SimulatedPipettorClient(TimeoutPolicy timeoutPolicy, 
    ...
    public async Task<HardwareCommandResponse> ExecuteCommandAsync(ApiCommandRequest request, CancellationToken cancellationToken = default)
    {
        EnsureConnected();
    ...
    public async Task<HardwareCommandAccepted> SubmitCommandAsync(ApiCommandRequest request, CancellationToken cancellationToken = default)
    {
        EnsureConnected();
        CleanupIdempotency();
    ...
    private async Task RunCommandAsync(ApiCommandRequest request, CancellationToken linkedToken)
    {
        try
        {
